...
int main() {
    ...
    // men.funcSameName();  // 默认情况下存在函数隐藏问题，子类无法调用父类同名成员函数（子类中如果有一个同名函数，那么父类中不管有多少个同名函数，子类中都无法访问到父类的同名函数）

    // 通过 using 关键字解决函数隐藏问题，让父类的同名函数在子类中可见，也就是让父类同名函数在子类中以重载的方式来使用
    men.funcSameName();
