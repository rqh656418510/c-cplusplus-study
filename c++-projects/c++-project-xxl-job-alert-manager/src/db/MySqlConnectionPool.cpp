...
std::shared_ptr<MySqlConnection> MySqlConnectionPool::getConnection() {
    ...
    while (true) {
        // 使用While循环来避免线程虚假唤醒，等待直到队列有可用连接
        while (this->connectionQueue_.empty()) {
            // 如果连接队列为空，唤醒生产线程创建新连接
            this->cv_.notify_all();
            // 如果连接队列为空，则等待指定的时间
        ...
        // 获取队头的连接
        MySqlConnection *pconn = this->connectionQueue_.front();

        // 移除队头的连接
        this->connectionQueue_.pop_front();

        // 通过Ping检查连接是否有效，如果无效则释放连接
        if (pconn->isBrokened() || (pconn->getIdleTotalTimes() >= this->maxIdleTime_ * 1000 && !pconn->ping())) {
            // 计算器减一
            this->connectionCount_--;
            // 释放连接
            delete pconn;
        ...
        // 连接有效，用智能指针包装后返回，自定义智能指针释放资源的方式，将连接归还到队列中
        std::shared_ptr<MySqlConnection> sp(pconn, [this](MySqlConnection *pconn) {
            // 获取互斥锁
            std::unique_lock<std::mutex> lock(this->queueMutex_);
