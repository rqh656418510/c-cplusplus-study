...
public:
    ...
    // 获取连接上次使用距今的时间间隔（毫秒）
    int64_t getLastUsedIntervalTime() const;

    // 刷新连接进入空闲状态的时间戳
    void refreshIdleStartTime();

    // 获取连接进入空闲状态的总时长（单位毫秒）
    int64_t getIdleTotalTimes() const;

    // 刷新连接上次发送心跳的时间戳
    void refreshLastHeartbeatTime();

    // 获取连接上次发送心跳距今的时间间隔（毫秒）
    int64_t getLastHeartbeatIntervalTime() const;
...
private:
    ...
    std::atomic<int64_t> lastUsedTime_{0};       // 记录连接上次使用的时间戳（单位毫秒）
    std::atomic<int64_t> idleStartTime_{0};      // 记录连接进入空闲状态的时间戳（单位毫秒）
    std::atomic<int64_t> lastHeartbeatTime_{0};  // 记录连接上次发送心跳的时间戳（单位毫秒）
