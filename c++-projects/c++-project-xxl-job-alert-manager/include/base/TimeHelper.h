...
public:
    ...
    static int64_t localToUtcTimestampSec(const std::string& datetime);

    // 将本地时间字符串（格式为 YYYY-MM-DD HH:MM:SS）转换为UTC时间戳（单位：毫秒）
    static int64_t localToUtcTimestampMs(const std::string& datetime);

    // 将本地时间字符串（格式为 YYYY-MM-DD HH:MM:SS.mmm）转换为UTC时间戳（单位：微秒）
    static int64_t localToUtcTimestampUs(const std::string& datetime);
};