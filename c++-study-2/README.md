## 章节概览

```
└── day01
    ├── demo01          第一个 C++ 程序
    ├── demo02          双冒号作用域限定运算符
    ├── demo03          命名空间的使用
    ├── demo04          using 声明与 using 编译指令的使用
    ├── demo05          C++ 对 const 关键字的增强
    ├── demo06          C++ 中的引用
    ├── demo07          C++ 中对数组建立引用
    ├── demo08          C++ 中引用作函数参数
    ├── demo09          C++ 中引用的注意事项
    ├── demo10          C++ 中引用的本质
    ├── demo11          C++ 中的指针引用
    ├── demo12          C++ 中的常量引用
└── day02
    ├── demo01          作业案例-设计圆类
    ├── demo02          作业案例-设计学生类
    ├── demo03          内联函数的使用
    ├── demo04          函数默认参数与函数占位参数
    ├── demo05          函数重载
    ├── demo06          extern 关键字浅析
    ├── demo07          C 语言封装与 C++ 封装的区别
└── day03
    ├── demo01          面向对象设计案例 - 设计立方体类
    ├── demo02          面向对象设计案例 - 点和圆的关系
    ├── demo03          构造函数和析构函数的使用
    ├── demo04          构造函数的分类及调用
    ├── demo05          拷贝构造函数的调用时机
    ├── demo06          构造函数的调用规则
    ├── demo07          深拷贝与浅拷贝
    ├── demo08          构造函数的初始化列表
    ├── demo09          类对象作为类成员的案例
    ├── demo10          explicit 关键字的作用
    ├── demo11          new 运算符的使用
└── day04
    ├── demo01          静态成员变量
    ├── demo02          静态成员函数
    ├── demo03          单例模式使用案例之一
    ├── demo04          单例模式使用案例之二
    ├── demo05          成员变量和成员属性分开处理
    ├── demo06          this 指针的使用
    ├── demo07          空指针访问成员函数
    ├── demo08          常函数与常对象的使用
    ├── demo09          全局函数作为友元函数
    ├── demo10          类模板的分文件编写的问题以及解决方案
    ├── demo11          整个类作为友元类
    ├── demo12          成员函数作为友元函数
└── day05
    ├── demo01          数组类的封装
    ├── demo02          加号运算符重载
    ├── demo03          左移运算符重载
    ├── demo04          前置后置递增运算符重载
    ├── demo05          指针运算符重载（实现智能指针）
    ├── demo06          赋值运算符重载
    ├── demo07          []运算符重载
    ├── demo08          关系运算符重载
    ├── demo09          函数运算符重载
    ├── demo10          强化训练字符串封装
    ├── demo11          继承的引出
    ├── demo12          继承方式
    ├── demo13          继承中的对象模型
    ├── demo14          继承中的构造和析构顺序
    ├── demo15          继承中的同名成员处理
    ├── demo16          继承中的静态成员处理
    ├── demo17          多继承的概念以及问题
    ├── demo18          菱形继承的问题以及解决方案
    ├── demo19          虚基类的内部工作原理解析
└── day06
    ├── demo01          静态联编和动态联编
    ├── demo02          多态原理解析
    ├── demo03          多态案例-计算器案例
    ├── demo04          抽象类和纯虚函数
    ├── demo05          虚析构函数
    ├── demo06          纯虚析构函数
    ├── demo07          向上类型转换和向下类型转换
    ├── demo08          多态案例二 - PK游戏开发
└── day07
    ├── demo01          函数模板的基础使用
    ├── demo02          使用函数模板实现通用的数组排序
    ├── demo03          函数模板与普通函数的区别以及调用规则
    ├── demo04          函数模板与函数重载
    ├── demo05          函数模板的局限性
    ├── demo06          类模板的基本使用
    ├── demo07          成员函数创建时机
    ├── demo08          类模板作为函数的参数
    ├── demo09          类模板碰到继承的问题以及解决
    ├── demo10          类模板的类外实现成员函数
    ├── demo11          类模板的分文件编写问题以及解决
    ├── demo12          友元碰到类模板-友元函数类内实现
    ├── demo13          友元碰到类模板-友元函数类外实现
    ├── demo14          类模板的应用-数组类的封装
└── day08
    ├── demo01          类型转换-静态类型
    ├── demo02          类型转换-动态类型
    ├── demo03          类型转换-静态和重新解释转换
```

## 常用命令

### Linux 编译与运行

``` shell
# 编译（可同时指定多个".cpp"文件，不需要指定".h"文件）
$ g++ helloworld.cpp -o helloworld

# 运行
$ ./helloworld
```

### Windows 编译与运行

在 Windows 系统的开始菜单栏里，找到 `Developer Command Prompt for VS 2019` 应用程序，双击运行后，在 `Command` 窗口内执行以下命令来编译 C++ 源文件：

```
# 编译（可同时指定多个".cpp"文件，不需要指定".h"文件）
> cl helloworld.cpp /EHsc

# 运行
> helloworld
```
