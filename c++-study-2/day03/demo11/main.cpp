...
void test01() {
    ...
    Person *p2 = new Person();  // 通过 new 运算符创建对象（动态），会在堆上开辟内存空间，对象所在的作用域结束后或者程序结束后都不会自动释放内存空间，即不会自动调用析构函数
...
void test03() {
    Person *pArray = new Person[3]; // 通过 new 运算符创建对象数组时，一定会自动调用类的默认构造函数，所以必须提供类的默认构造函数，否则编译不通过
