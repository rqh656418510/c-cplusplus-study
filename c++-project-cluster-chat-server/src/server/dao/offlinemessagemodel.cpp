...
vector<OfflineMessage> OfflineMessageModel::select(int userId) {
    ...
    // 拼接 SQL 语句
    sprintf(sql, "select message, createtime from offlinemessage where userid = '%d' order by createtime desc", userId);
    ...
    if (mysql.connect()) {
        ...
        if (res != nullptr && mysql_num_rows(res) > 0) {
            ...
            while ((row = mysql_fetch_row(res)) != nullptr) {
                ...
                long createtime = atol(row[1]);
                results.emplace_back(message, createtime);
            }
            // 释放资源
